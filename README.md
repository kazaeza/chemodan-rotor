...
**Через Node (как на Railway):**
```bash
npm install
npm run dev
```

**Вообще без сервера:** открыть `index.html` двойным кликом — тоже работает.
...
- Презентация *сама по себе* = твоя идея №2 (Чемодан в РПЛ). Метафора работает на
  форму подачи.
- Два замка — интеракция, а не автоплей. Читатель должен дотронуться.
- Прогресс в виде штампов на крышке (не циферки «3/5»).
- Артефакты разбросаны по бархату под разными углами — не сетка, а музейный хаос.
- Финал — не «слайд спасибо», а багажная квитанция с печатью «доставлено», падающая
  с привязанной верёвкой.
- Кнопка копирования TG в финале — единственный call-to-action.
